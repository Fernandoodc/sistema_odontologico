...
from database.models import Doctores, Citas, Pacientes, EstadosCiviles
...
async def viewAgenda(request: Request, fecha:str = datetime.now().strftime("%Y-%m-%d"), idDoctor:int = None,  user=Depends(manager), db : Session = Depends(get_db)):
    ...
    pacientes = db.query(Pacientes).limit(10).all()
    drs = db.query(Doctores.id_doctor, User.nombre, User.apellido).join(User).filter(User.id_tipo == 2, User.activo == True).limit(10).all()
    estCiviles = db.query(EstadosCiviles).all()
    if idDoctor == None:
        citas = db.query(Citas.hora, Citas.duracion, Pacientes.id_paciente, Pacientes.nombre, Pacientes.apellido, Pacientes.celular, User.nombre, User.apellido).join(Pacientes).join(Doctores).join(User).filter(Citas.fecha == fecha).all()
    else:
        citas = db.query(Citas.hora, Citas.duracion, Pacientes.id_paciente, Pacientes.nombre, Pacientes.apellido, Pacientes.celular, User.nombre, User.apellido).join(Pacientes).join(Doctores).join(User).filter(Citas.fecha == fecha, Doctores.id_doctor == idDoctor).all()
    return templates.TemplateResponse('agenda.html', context={'request': request, 'userInfo': user, 'fecha': fecha, 'doctores': drs, 'citas': citas, 'pacientes': pacientes, 'estCiviles': estCiviles, 'drSelected' : idDoctor})
